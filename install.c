/*
 * PS5 App Installer for slopkit
 * Based on the implementation in itsPLK/ps5-payload-manager
 */
...
int main(int argc, char *argv[]) {
  ...
  if (stat(base_dir, &st) == 0) {
    printf("Updating existing app launcher (%s)...\n", title_id);
  } else {
    printf("Installing browser launcher app (%s)...\n", title_id);
  }
  ...
  if (mkdir(base_dir, 0755) && errno != EEXIST) {
    printf("Failed to create app dir: %s (errno: %d)\n", base_dir,
           errno);
  ...
  if (mkdir(sce_sys_dir, 0755) && errno != EEXIST) {
    printf("Failed to create sce_sys dir: %s (errno: %d)\n",
           sce_sys_dir, errno);
    sceAppInstUtilTerminate();
    return -1;
  }
  ...
  if (install_file(param_path, param_json, param_json_size)) {
    printf("Failed to install param.json\n");
    sceAppInstUtilTerminate();
    return -1;
  }

  if (install_file(icon_path, icon0_png, icon0_png_size)) {
    printf("Failed to install icon0.png\n");
    sceAppInstUtilTerminate();
    return -1;
  }
  ...
  printf("Launcher app installed successfully.\n");
